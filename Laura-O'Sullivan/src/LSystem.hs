...
module LSystem (
  display, displayAll,
  Command (..),
  Pen (..), black, white, red, green, blue, pink, orange, yellow, turquoise, purple,
  Distance, Angle,
  triangle, tree,
...
white, black, red, green, blue, pink, orange, yellow, turquoise, purple:: Pen
white = Colour 1.0 1.0 1.0
black = Colour 0.0 0.0 0.0
red   = Colour 1.0 0.0 0.0
green = Colour 0.0 1.0 0.0
blue  = Colour 0.0 0.0 1.0
pink  = Colour 0.99 0.078 0.403
orange = Colour 0.99 0.645 0.0
yellow = Colour 1.0 1.0 0.0
turquoise = Colour 0.2 1.0 1.0
purple = Colour 0.6 0.2 1.0
...
instance Arbitrary Pen where
    arbitrary  =  sized pen
        where
          pen _  =  elements [black,red,green,blue,white,pink,orange,yellow,turquoise,purple,Inkless]
...
wrapSvgTag x = "<svg height=\"" ++ "100%" ++ "\" width=\"" ++ "100%" ++ "\">" ++ x ++ "</svg>"

display' :: String -> Command -> IO ()
display' fname = writeFile (fname ++ ".html") . wrapSvgTag . toHtmlBody

display :: Command -> IO ()
display = display' "output"

displayAll :: [(String, Command)] -> IO ()
displayAll [] = return ()
displayAll ((fn, cmd):xs) = display' fn cmd >> displayAll xs
